...
export default class ProfileRepository {
  async fetchSavedPrompts(key: string | null): Promise<Array<Prompt>> {
    const values = Object.entries(await chrome.storage.local.get(key)).map(
      ([key, value]) => {
        try {
  ...
  async getSavedPrompts(): Promise<Array<Prompt>> {
    return (await this.fetchSavedPrompts(null)).filter(
      (v) => !v.isNegativePrompt
    );
  }
      ...
      func: (prompt: string, isNegative: boolean) => {
        const element = (() => {
          if (isNegative) {
            ...
            return document
              .querySelector("body > gradio-app")
              ?.shadowRoot?.querySelector("#txt2img_prompt > label > textarea");
          }
        })();
